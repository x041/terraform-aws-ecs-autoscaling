resource "aws_appautoscaling_target" "default" {
  count              = module.this.enabled && !var.ignore_changes_min_max_capacity ? 1 : 0
  max_capacity       = var.max_capacity
  min_capacity       = var.min_capacity
  resource_id        = "service/${var.cluster_name}/${var.service_name}"
  role_arn           = "arn:aws:iam::${module.this.aws_account_id}:role/aws-service-role/ecs.application-autoscaling.amazonaws.com/AWSServiceRoleForApplicationAutoScaling_ECSService"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
}

resource "aws_appautoscaling_target" "ignore_changes_min_max_capacity" {
  count              = module.this.enabled && var.ignore_changes_min_max_capacity ? 1 : 0
  max_capacity       = var.max_capacity
  min_capacity       = var.min_capacity
  resource_id        = "service/${var.cluster_name}/${var.service_name}"
...
resource "aws_appautoscaling_policy" "default" {
  ...
  scalable_dimension = try(aws_appautoscaling_target.default[0], aws_appautoscaling_target.ignore_changes_min_max_capacity[0]).scalable_dimension
  service_namespace  = try(aws_appautoscaling_target.default[0], aws_appautoscaling_target.ignore_changes_min_max_capacity[0]).service_namespace
